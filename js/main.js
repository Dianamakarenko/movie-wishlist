...
// function removeFromWishList(arr, name){
//     let indexToDelete = arr.findIndex(film => film === name)
//     if(indexToDelete !== -1){
...
// function editWishList(arr, nameToEdit, newName){
//     let indexToDelete = arr.findIndex(film => film === nameToEdit)
//     if(indexToDelete !== -1){
//         arr.splice(indexToDelete,1, newName)
//     } else {
//         alert(nameToEdit + ' is not defined')
//     }
//     return arr
// }
...
function searchMovie(query, year, type) {
    let users = fetch(`https://www.omdbapi.com/?apikey=${API_KEY}&s=${query}&y=${year}&type=${type}`)
      .then(response => {
        if(response.ok) {
      ...
      .then(() => document.querySelector(`.loader`).classList.remove(`--visible`))
      .catch(err => {
...
document.querySelector(`#searchForm`).onsubmit = function (event) {
    ...
    let elements = document.forms.searchForm.elements;

    searchMovie(elements.search.value, elements.year.value, elements.type.value)
}
